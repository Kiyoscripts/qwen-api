...
export function KeysPanel() {
  ...
  return (
    <>
      ...
      <section className="mt-7 border border-rule bg-[var(--paper-2)] p-5" aria-labelledby="quickstart-title"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">Quick start</p><h2 id="quickstart-title" className="mt-1 text-xl text-ink">Your first API request</h2><p className="body mt-2 text-sm">Create a key, choose an available model, then test it in the request laboratory.</p></div><span className="font-mono text-xs text-ink-3">{fresh?"Step 2 of 3":keys?.some(k=>!k.revoked)?"Ready":"Step 1 of 3"}</span></div><ol className="mt-5 grid gap-3 md:grid-cols-3"><li className={`border p-4 ${keys?.some(k=>!k.revoked)||fresh?"border-emerald-700":"border-signal"}`}><span className="font-mono text-[10px] uppercase text-ink-3">01 · API key</span><p className="mt-2 text-sm text-ink">{keys?.some(k=>!k.revoked)||fresh?"Key created":"Create a key below"}</p></li><li className="border border-rule p-4"><span className="font-mono text-[10px] uppercase text-ink-3">02 · Model</span><p className="mt-2 text-sm text-ink">Choose a live model</p><Link href="/models" className="mt-2 inline-block font-mono text-xs text-signal underline underline-offset-2">Browse models</Link></li><li className="border border-rule p-4"><span className="font-mono text-[10px] uppercase text-ink-3">03 · Request</span><p className="mt-2 text-sm text-ink">Run a completion</p><Link href="/playground" className="mt-2 inline-block font-mono text-xs text-signal underline underline-offset-2">Open playground</Link></li></ol><p className="mt-4 text-xs text-ink-3">Your plaintext key appears only once. Copy it before closing the key banner.</p></section>
