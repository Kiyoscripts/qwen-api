...
export interface Sample { lang: string; code: string; }
    ...
    <div className="border border-rule bg-[var(--paper)]" style={{ borderRadius: "var(--r-sm)" }}>
      <div className="flex items-center justify-between border-b border-rule">
        <div role="tablist" aria-label="Language" className="flex">
          {samples.map((s, i) => (
            <button key={s.lang} role="tab" aria-selected={i === active} onClick={() => setActive(i)}
              className={`border-r border-rule px-4 py-2.5 font-mono text-[12px] transition-colors duration-200 ${
                i === active ? "bg-[var(--paper-2)] text-ink" : "text-ink-3 hover:text-ink"}`}>
              {s.lang}
            </button>
          ))}
        </div>
      ...
      <pre className="overflow-x-auto px-5 py-5">
        <code className="font-mono text-[12.5px] leading-[1.7] text-ink">{samples[active].code}</code>
      </pre>
    </div>
  );
}
